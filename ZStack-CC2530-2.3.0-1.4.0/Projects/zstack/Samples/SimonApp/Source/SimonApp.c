...
/*added by Simon*/
#define UART_MAX 112
unsigned char uartbuf[UART_MAX]={0};
...
void SimonApp_SendTheSerial(void);
...
//                      SimonApp_SEND_MSG_TIMEOUT );
  ...
  /*added  by Simon*/
   if ( events & Serial_CallBack_EVT )
   {
     SimonApp_SendTheSerial();
     return (events ^ Serial_CallBack_EVT);
   }
...
//#endif
      osal_memcpy(uartbuf,(unsigned char*)pkt->cmd.Data,pkt->cmd.DataLength);
      HalUARTWrite(0,uartbuf,pkt->cmd.DataLength);
      osal_memset(uartbuf,0,UART_MAX);
      break;
  }
}
...
void SimonApp_SendTheSerial( void )
{
  ...
  if(uartbuf[UART_MAX-1] != '\0')
  {
     len = UART_MAX; 
  }else{
     len = osal_strlen( (char *)uartbuf ); 
  }
  
  my_DstAddr.addrMode = (afAddrMode_t)Addr16Bit; 
  my_DstAddr.endPoint = SimonApp_ENDPOINT;
  my_DstAddr.addr.shortAddr = 0x0000;
  

  if ( AF_DataRequest( &my_DstAddr, &SimonApp_epDesc,
                       SimonApp_CLUSTERID,
                       len,
                       uartbuf,
                       &SimonApp_TransID,
...
{
    osal_memset(uartbuf,0,UART_MAX);
    HalUARTRead(0,uartbuf,UART_MAX);
//    if(osal_memcmp(uartbuf,"lzj",3))
//    {
//      HalUARTWrite(0,uartbuf,3);
//    }
    if(DEV_FLAG == 3)
      osal_set_event(SimonApp_TaskID,Serial_CallBack_EVT);
}
